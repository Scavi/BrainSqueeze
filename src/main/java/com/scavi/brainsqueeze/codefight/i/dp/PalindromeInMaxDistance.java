...
public class PalindromeInMaxDistance {
     ...
     */
    public boolean isPalindrome(final String input, final int maxDiff) {
        if (input.length() < 2) {
            return true;
        } else if (maxDiff < 0) {
            return false;
        ...
        for (int i = 1; i < testPalindrome1.length(); ++i) {
            for (int j = 1; j < testPalindrome2.length(); ++j) {
                if (testPalindrome1.charAt(i) == testPalindrome2.charAt(j)) {
                    dpCache[i][j] = Math.min(dpCache[i - 1][j - 1], Math.min(dpCache[i - 1][j], dpCache[i][j - 1]));
                } else {
                    dpCache[i][j] = Math.min(dpCache[i - 1][j - 1], dpCache[i - 1][j]) + 1;
                }
            }
        }
